...
describe('Authentication Integration Tests', () => {
    ...
    it('deve retornar 401 se o header de autorização não for fornecido aqui', () => {
        ...
        const req = { headers: {} };
        const res = {
            status: jest.fn().mockReturnThis(), // Permite encadear com .json()
            json: jest.fn()
        };
        const next = jest.fn();
        ...
        // Opcional: Certifica-se de que o next não foi chamado
    ...
    beforeAll(async () => {
        ...
        for (const role of userTypes) {
            // Gerar uma senha hash para o teste
    ...
    afterAll(async () => {
        await UserModel.destroy({ 
            where: {},
            force: true // Isso garante que os registros sejam realmente removidos
        });
    });
